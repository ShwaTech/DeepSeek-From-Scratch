...
├── 01_prepare.py          # Data preparation pipeline (Listings 6.2–6.5)
├── 02_model.py            # Complete MiniDeepSeek architecture (Listings 6.6–6.18)
├── 03_train.py            # Training loop with evaluation (Listings 6.19–6.24)
└── 04_sample.py           # Text generation from trained model
```
...
- **01_prepare.py** — Downloads TinyStories from Hugging Face, tokenizes with the `gpt2` BPE tokenizer (tiktoken), saves as memory-mapped binary files for efficient training
- **02_model.py** — The complete MiniDeepSeek architecture integrating:
...
- **03_train.py** — Training script with cosine LR scheduling, mixed-precision training, and periodic checkpointing (~130M parameter "flagship" configuration)
- **04_sample.py** — Interactive text generation using KV-cached inference
